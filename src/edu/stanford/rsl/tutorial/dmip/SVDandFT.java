...
public class SVDandFT {
	...
	public static void optimizationProblem3(Grid2D image, int rank)
	{
		...
		{
			SimpleVector us =
			svd.getU().getCol(k).multipliedBy(svd.getSingularValues()[k]);
		...
		SimpleMatrix usK = SimpleOperators.multiplyMatrixProd(svd.getU().getSubMatrix(0,
		0, svd.getU().getRows(),rank), svd.getS().getSubMatrix(0, 0, rank, rank));
		SimpleMatrix IapproxK = SimpleOperators.multiplyMatrixProd(usK,
		svd.getV().getSubMatrix(0,0,svd.getV().getRows(), rank). transposed());
	...
	public static void fourierExercise(Grid2D image)
	{
		...
		Grid2D imageTransf = imageC.getMagnSubGrid(0,0, image.getWidth(),
				image.getHeight());
		imageTransf.show();
	}
