...
public class DefectPixelInterpolation {
	...
	public Grid2D interpolateSpectral(Grid2D image, Grid2D mask, int maxIter, boolean zeroPadding)
	{
		...
		for(int i = 0; i < maxIter; i++)
		{
			...
			int t2 = (t1>0) ? dim[1] - (t1 % dim[1]): t1;//TODO
			...
			else
			{
				...
				Complex res_s1t1 = FHatNextVal_s1t1.add(val);//HINT
				Complex res_s2t2 = FHatNextVal_s2t2.add(val.getConjugate());//HINT
		...
		for (int i=0; i < result.getWidth(); i++){
			for (int j = 0; j < result.getHeight();j++){
				if(mask.getAtIndex(i,j)==0){
					result.setAtIndex(i,  j, FHat.getAtIndex(i,j));
	...
	public static void main(String[] args) {
		...
		String filename = "C:/DMIPpics/testimg.bmp";
		String filenameMask = "C:/DMIPpics/mask.bmp";
